...
public class UndirectionalLinkDelayDescriptorSubTLV extends BGP4TLVFormat{
    ...
    */
	...
	public void encode() {
		...
		/*
		int di=Float.floatToIntBits(delay);

		this.tlv_bytes[offset ] = (byte)(di >>> 24);
		this.tlv_bytes[offset + 1] = (byte)(di >> 16 & 0xff);
		this.tlv_bytes[offset + 2] = (byte)(di >> 8 & 0xff);
		this.tlv_bytes[offset + 3] = (byte)(di & 0xff);
		*/
	}
	public void decode(){
		...
		int di = 0;
		for (int k = 0; k < 4; k++) {
			di = (di << 8) | (this.tlv_bytes[k+4] & 0xff);
		}
		//this.delay=Float.intBitsToFloat(di);
	}
